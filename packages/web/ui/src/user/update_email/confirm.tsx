	...
	return (
		<Card className="vulpo-auth-user-email-reject">
			<header className="vulpo-card-header">
				<h3 className="vulpo-auth-card-title">{t.update_email_confirm.title}</h3>
			</header>
			...
			{ (ok && submitted) &&
				<Fragment>
					...
					<p className="vulpo-auth-text-success">
						{ t.update_email_confirm.submitted }
					</p>
				</Fragment>
			}
		</Card>
...
let ConfirmContainer = () => {
	...
	let handleSubmit = useCallback(() => {
		...
		auth.confirmUpdateEmail(id, token)
